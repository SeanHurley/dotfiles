function fish_prompt --description 'Write out the prompt'
    ...
    if test (git rev-parse > /dev/null 2>&1;echo $status) -eq 0
      ...
      if not set -q __git_mod
          set __git_mod (set_color red)(git ls-files -m ^/dev/null | wc -l)(set_color normal)""
      end

      if not set -q __git_unt
          set __git_unt (set_color yellow)(git ls-files --other --exclude-standard | wc -l)(set_color normal)""
      end

      if not set -q __git_staged
          set __git_staged (set_color green)(git diff --name-only --cached | wc -l)(set_color normal)""
      end

      if not set -q __git_ahead
          set -lx ahead (git rev-list --left-right origin/HEAD...HEAD | grep \> -c)
      ...
      if not set -q __git_behind
          set -lx behind (git rev-list --left-right origin/HEAD...HEAD | grep \< -c)
